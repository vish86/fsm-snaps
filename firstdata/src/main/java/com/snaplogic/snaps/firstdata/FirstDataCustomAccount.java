...
public class FirstDataCustomAccount implements Account<AccountBean> {
    ...
    public void defineProperties(PropertyBuilder propertyBuilder) {
        propertyBuilder.describe(APP_ID, APP_ID_LABEL, APP_ID_DESC)
                .required()
                .add();
        propertyBuilder.describe(SERVICE_ID, SERVICE_ID_LABEL, SERVICE_ID_DESC)
                ...
                .required()
                .add();
        propertyBuilder.describe(SERVICE_URL, SERVICE_URL_LABEL, SERVICE_URL_DESC)
                .required()
                .add();
        propertyBuilder.describe(WSDL_URL, WSDL_URL_LABEL, WSDL_URL_DESC)
                .required()
                .add();
        propertyBuilder.describe(DATAWIRE_ID, DATAWIRE_ID_LABEL, DATAWIRE_ID_DESC)
                .required()
                .add();
        propertyBuilder.describe(GROUP_ID, GROUP_ID_LABEL, GROUP_ID_DESC)
                .required()
                .add();
        propertyBuilder.describe(TERMINAL_ID, TERMINAL_ID_LABEL, TERMINAL_ID_DESC)
                .required()
                .add();
        propertyBuilder.describe(AUTH_STRING, AUTH_STRING_LABEL, AUTH_STRING_DESC)
                .required()
                .add();
        propertyBuilder.describe(TIMEOUT, TIMEOUT_LABEL, TIMEOUT_DESC)
                .required()
                .type(SnapType.INTEGER)
                .defaultValue(30)
                .add();
    ...
    public AccountBean connect() throws ExecutionException {
        return new AccountBean().setAppID(appID)
                .setServiceID(serviceID)
                .setServiceURL(serviceURL)
                .setServiceWSDLURL(serviceWSDLURL)
                .setDatawireId(datawireId)
                .setGroupId(groupId)
