...
/**
 * Provides functionality to suggest a template for a SOAP request. The template will provide
 * envelope, header and operation. The template will be returned to the caller. The caller can edit
 * the template afterwards if needed or use it directly as part of the request.
 *
 * @author svatada
 */
public class TemplateSuggestionsImpl implements Suggestions {
    ...
    public TemplateSuggestionsImpl(final EditorSuggestionProvider suggestionsProvider,
    ...
    public void suggest(final SuggestionBuilder suggestionBuilder,
        ...
        String xmlTemplate = ((SoapEditorSuggestionsProviderImpl) suggestionsProvider).withClientBuilder(
                clientBuilder)
                .withHttpContextProvider(null)
                .suggestContent();
