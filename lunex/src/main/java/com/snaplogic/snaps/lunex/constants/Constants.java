...
public class Constants {
	...
	public static enum CResource {
		Account("Account"), NewOrder("NewOrder"), PreOrder("PreOrder"), VoidTx(
		        "VoidTx");
	...
	public static enum DResource {
		DeleteSpeedDial("DeleteSpeedDial"), DeleteRegisterPhone(
		        "DeleteRegisterPhone"), RemoveCustomer("RemoveCustomer");
	...
	public static enum RResource {
		GetAccount("GetAccount"), ListAccount("ListAccount"), ListSpeedDial(
		        "ListSpeedDial"), ListRegisterPhone("ListRegisterPhone"), GetOrderStatus(
		        "GetOrderStatus"), ListProduct("ListProduct"), GetSeller(
		        "GetSeller"), ListTransactions("ListTransactions"), ListDidByState(
		        "ListDidByState"), GetAutoITU("GetAutoITU"), TopUpOrder(
		        "TopupOrder");
	...
	public static final ImmutableMap<String, Integer> REQ_BODY_PARAM_INFO = new ImmutableMap.Builder<String, Integer>()
	        .put("AutoRefillUpdatedBy", 0).put("AccessPhone", 0)
	        .put("AccessPhoneInt", 0).put("Balance", 0).put(CITY, 0)
	        .put("Created", 0).put("CreatedBy", 0).put("Currency", 0)
	        .put("Description", 0).put("Distributor", 0).put("Email", 0)
	        .put("ExpiredDate", 0).put(FIRST_NAME, 0)
	        .put("FirstPurchaseDate", 0).put("FirstUseDate", 0)
	        .put(LAST_NAME, 0).put("LastPurchaseDate", 0).put("LastUseDate", 0)
	        .put("MinBalance", 0).put("Minutes", 1).put("Num", 1)
	        .put("Notes", 0).put(PHONE2, 1).put("Pin", 1).put("Region", 0)
	        .put("RedeemDate", 0).put("ReferralName", 0)
	        .put("ReferralPhone", 1).put("RefillAmt", 0).put("Reseller", 0)
	        .put("RegionInt", 1).put("Sku", 1).put(STATE, 0)
	        .put("StatementOption", 0).put("Status", 0).put(STREET, 0)
	        .put("Updated", 0).put("UpdatedBy", 0).put(ZIPCODE, 0).build();

	/*
	 * POST/Create operation supported resource list
	 */
	public static final ImmutableMap<String, ImmutableSet<String>> CR_PARAM_LIST = ImmutableMap
	        .of(CResource.NewOrder.toString(), ImmutableSet.of(SELLER, CID,
	                SKU, PHONE, PROMO_PHONE, AMOUNT, QUANTITY, PHONETYPE, LANG,
	                NOTIFY, MERCHANTNAME, MERCHANTID), CResource.Account
	                .toString(), ImmutableSet.of(SELLER, PHONETYPE, LANG,
	                NOTIFY), CResource.VoidTx.toString(), ImmutableSet.of(
	                SELLER, TXID, NOTIFY), CResource.PreOrder.toString(),
	                ImmutableSet.of(SELLER, CID, SKU, PHONE, AMOUNT));

	public static final ImmutableSet ADDRESS_JSONOBJ = ImmutableSet.of(CITY, COUNTRY,
	        STATE, STREET,SUITE,ZIPCODE);
	...
	public static final ImmutableMap<String, ImmutableSet<String>> RR_PARAM_LIST = new ImmutableMap.Builder<String, ImmutableSet<String>>()
	        .put(RResource.GetAccount.toString(),
	                new ImmutableSet.Builder<String>().add(SELLER).add(CID)
	                        .add(SKU).add(PHONE).add(PROMO_PHONE).add(AMOUNT)
	                        .add(QUANTITY).add(PHONETYPE).add(LANG).add(NOTIFY)
	                        .add(MERCHANTNAME).add(MERCHANTID).build())
	        .put(RResource.GetOrderStatus.toString(),
	                new ImmutableSet.Builder<String>().add(SELLER)
	                        .add(PHONETYPE).add(LANG).add(NOTIFY).build())
	        .put(RResource.GetSeller.toString(),
	                new ImmutableSet.Builder<String>().add(SELLER).add(TXID)
	                        .add(NOTIFY).build())
	        .put(RResource.ListAccount.toString(),
	                new ImmutableSet.Builder<String>().add(SELLER).add(CID)
	                        .add(SKU).add(PHONE).add(AMOUNT).build())
	        ...
	        .put(RResource.GetAutoITU.toString(),
	                new ImmutableSet.Builder<String>().add(SELLER).add(TYPE)
	                        .add(PHONE).build())
	        .put(RResource.TopUpOrder.toString(),
	                new ImmutableSet.Builder<String>().add(SELLER).add(ORDER)
	                        .add(SKU).add(PHONE).add(AMOUNT).build()).build();
