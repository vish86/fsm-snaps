...
public abstract class BaseService extends SimpleSnap implements
    ...
    @Override
    public Module getManagedAccountModule(
        final Account<AccountBean> account) {
    ...
    protected void process(
        Document document, String inputViewName) {
        ...
        try {
            ...
            if (RequestProcessor.getInstance()
                ...
                map = new HashMap<String, Object>();
                map.put(ERROR_TAG, HTTP + RequestProcessor.getInstance()
                    .getStatusCode());
                map.put(MESSAGE_TAG, response);
                map.put(REASON_TAG, HTTP_ERROR_REASON);
                map.put(RESOLUTION_TAG, HTTP_ERROR_RESOLUTION);
                snapException =
                    new SnapDataException(documentUtility.newDocument(map), response).withReason(
                        HTTP_ERROR_REASON)
            ...
            snapException =
                new SnapDataException(documentUtility.newDocument(map), ex.getMessage()).withReason(
                    ILLEGAL_STATE_REASON)
    ...
    private String prepareJson(
        List<Pair<String, ExpressionProperty>> requestContent, Document document) {
        ...
        if (requestContent != null) {
            ...
            return OPENTAG + subJson + json.deleteCharAt(json.length() - 1)
                .toString() + CLOSETAG;
        }
        return "";
    }

    private String getJsonSlice(
        Pair<String, ExpressionProperty> paramPair, Document document) {
        String key = paramPair.getKey();
        if (REQ_BODY_PARAM_INFO.get(key) == 1) {
            return QUOTE + key + QUOTE + COLON + (String) paramPair.getRight()
                .eval(document) + COMMA;
        }
        return QUOTE + key + QUOTE + COLON + QUOTE + (String) paramPair.getRight()
            .eval(document) + QUOTE + COMMA;
    }

    private void setAuthHeaderProp(
        String username, String passcode) {
        headersProperties.add(Pair.of(AUTHORIZATION,
            BASIC + Base64.encodeBase64String((username.concat(COLON).concat(passcode)).getBytes())));
    }
